...
namespace EspIoTHub.UWP.VoiceCommandService
{
	public class EspIoTCommandService : IBackgroundTask
	{
		/// <summary>
		...
		public async void Run(IBackgroundTaskInstance taskInstance)
		{
			...
			if (triggerDetails != null && triggerDetails.Name == "EspIoTCommandService")
			{
				try
				{
					...
					switch (voiceCommand.CommandName)
					{
						case "showConversation":
							var destination = voiceCommand.Properties["state"][0];
		...
		{
			var userMessage = new VoiceCommandUserMessage();
			userMessage.SpokenMessage = cortanaResourceMap.GetValue("Launching", cortanaContext).ValueAsString;
